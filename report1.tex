...
The following histogram was obtained for the Wikipedia editor network:

\includegraphics[scale=0.6]{plots/206_histogram}

The source code in Julia that was used to create this image is as follows:

\begin{lstlisting}
"If you don't understand this you won't pass the graphical models course"
		David Barber on Julia
\end{lstlisting}
\hilight{David, insert source code here ..}
...
The two graphs given are Markov equivalent because they have the same sets of 
immoralities. Assuming nodes are indexed $1-9$, the immoralities found in both
graphs are:
\begin{itemize}
 \item $3 \leftarrow 1,2$
 \item $3 \leftarrow 1,5$
 \item $9 \leftarrow 4,7$
\end{itemize}

The code that was used for calculating these is:
\begin{lstlisting}
    ...
    for k=1:length(parentsA)
        for l=k+1:length(parentsA)
            if (undirA(parentsA(k), parentsA(l)) == 0)
                ...
                parentsExist = ismember(parentsA(k), parentsB) && 
ismember(parentsA(l), parentsB);
...
\hilight{re-factor the code so that immoralities are computed for each node 
individually}
